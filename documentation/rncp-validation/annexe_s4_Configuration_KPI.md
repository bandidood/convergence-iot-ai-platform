# ANNEXE S.4 - CYBER RANGE ENVIRONMENT SETUP

## Spécifications Techniques Environnement Virtuel SCADA Eau

---

## S.4.1 ARCHITECTURE CYBER RANGE INFRASTRUCTURE CRITIQUE

### Vision Stratégique de l'Environnement d'Entraînement

Le Cyber Range de la station Traffeyère constitue un environnement d'entraînement cybersécurité de niveau industriel, reproduisant fidèlement l'architecture convergente IT/OT d'une infrastructure critique du secteur eau. Cette plateforme de simulation avancée permet la formation continue des équipes sécurité, la validation des procédures incident response et l'évaluation empirique de l'efficacité des mesures de protection déployées.

L'environnement virtuel intègre les technologies critiques identifiées dans l'analyse EBIOS RM : systèmes SCADA Schneider Electric, HMI Wonderware, automates programmables Siemens S7, protocoles industriels (Modbus/TCP, DNP3, OPC-UA) et infrastructure de supervision centralisée. Cette réplique haute-fidélité garantit un niveau de réalisme opérationnel indispensable à la formation des équipes en situation de crise cyber.

L'approche cyber range s'inscrit dans une démarche de cybersécurité proactive, transformant les investissements formation en capabilities opérationnelles mesurables. L'objectif stratégique consiste à développer une expertise interne de niveau expert, réduisant la dépendance aux prestataires externes et optimisant les temps de réponse incident de 65% selon les retours d'expérience sectoriels.

---

## S.4.2 SPÉCIFICATIONS TECHNIQUES INFRASTRUCTURE
